...
static I4 indexing_scan_folder(const char *dirname)
{
	...
	else
	{
		...
		InsertFolderPathInfo(gi4FolderPathID+1,gau1CurrentPath);
		InsertFolderInfo(gi4FolderID,dirname,gi4FolderPathID+1);
		gi4FolderPathID++;
		LOG_STR_RES("=================================================================");
		if (D_FOLDER_CNT_MAX <= gi4TotalFolderCount)
		{
	...
	else
	{
		...
		{	
			...
			if (get_fileextensiontype((U1*)pstDirEnt->d_name,&pu4ExtType) == 0)
			{
				...
				InsertFileInfo(gi4FileID,(const char*)pstDirEnt->d_name,gi4FolderPathID+1);
				if (D_FILE_CNT_MAX <= gi4TotalFileCount)
				{
					i4Result = D_OK;
					break;
